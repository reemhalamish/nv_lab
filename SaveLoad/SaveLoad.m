classdef SaveLoad < Savable & EventSender
    ...
    properties(Constant = true)
        ...
        EVENT_LOAD_SUCCESS_LOCAL_TO_SYSTEM = 'fileLoadedSuccessfullyToSystem';
            ...
            if (ischar(newNotes))
                if ~strcmp(newNotes, obj.mNotes)
                    ...
                    if isstruct(obj.mLocalSaveStruct)
                        % update the struct and the file!
    ...
    methods(Access = private)
        ...
        function loadedStruct = tryLoadingStruct(obj, fileFullPath, shouldSendErrors)
            ...
            structmCategory = structToLoad.(Savable.PROPERTY_CATEGORY);
            if ~strcmp(structmCategory, obj.mCategory)
                if shouldSendErrors
                    obj.sendWarning(sprintf(' can''t load file - mCategory mismatch! ignoring.\n(file mCategory: "%s", SaveLoad mCategory: "%s"', structmCategory, obj.mCategory));
                end
                return
            end
