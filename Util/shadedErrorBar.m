function varargout=shadedErrorBar(varargin)
% Generate continuous error bar area around a line plot
% 
% shadedErrorBar(x, y, errBar)
% shadedErrorBar(axes,___)
% H = shadedErrorBar(___)
%
% Purpose 
...
% Adapted by Nadav Slotky - November 2017
...
%If first parameter is of type axes, plot on it
v1 = varargin{1};
if isgraphics(v1)
    switch v1.Type
        case 'axes'
        ...
        case 'figure'
            ax = v1.Children(1);
        otherwise
            error('Can''t create plot in given graphic object');
    end
    varargin(1) = [];   % remove axes from varargin
else
    ax = gca;
end
...
%Log the hold status so we don't change it
initialHoldStatus=ishold;
if ~initialHoldStatus, hold on,  end

H = makePlot(ax,x,y,errBar,lineProps,transparent,patchSaturation);
...
function H = makePlot(ax,x,y,errBar,lineProps,transparent,patchSaturation)
